...
exports.reverseTxid = reverseTxid;
exports.isTxidField = isTxidField;
exports.executeInternalRPC = executeInternalRPC;
 ...
 */
async function executeMultipleRPC(requests) {
 ...
 */
function processAddressInfo(utxosJson, protorunesJson, ordOutputsJson, ordHeight, metashrewHeight) {
    ...
    (utxosJson || []).forEach((utxo) => {
        ...
        // Categorize UTXO
        const hasAssets = (runes && runes.length > 0) ||
 ...
 */
async function getAddressInfo(request) {
    ...
    try {
        ...
        const ordBlockHeight = results[results.length - 2];
        const metashrewHeight = parseInt(results[results.length - 1]);
        // If only one address, process normally
        if (uniqueAddresses.length === 1) {
            const [utxosResult, protorunesResult, ordOutputsResult] = results;
            const addressInfo = processAddressInfo(utxosResult, protorunesResult, ordOutputsResult, ordBlockHeight, metashrewHeight);
            return addressInfo;
        }
