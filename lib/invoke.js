...
exports.encodeAlkaneStorageRequestString = encodeAlkaneStorageRequestString;
    ...
    switch (k) {
        ...
        case "receive_intent":
            let receive_intent = {
                event: "receive",
                data: {
                    alkanes: v[k].incomingAlkanes.map(v => toAlkaneTransfer(v)),
                }
            };
        ...
        case "value_transfer":
            let value_transfer = {
                event: "transfer",
                data: {
                    alkanes: v[k].transfers.map(v => toAlkaneTransfer(v)),
                }
            };
...
function encodeUnwrapsRequest({ block, }) {
    ...
    return ("0x" +
        Buffer.from(new alkanes_1.alkanes.PendingUnwrapsRequest(input).serializeBinary()).toString("hex"));
}
function decodeUnwrapsResponse(hex) {
    const res = alkanes_1.alkanes.PendingUnwrapsResponse.deserializeBinary(new Uint8Array(Buffer.from((0, utils_1.stripHexPrefix)(hex), "hex")));
...
function decodeBlockResponse(hex) {
    return (0, utils_1.addHexPrefix)(Buffer.from(alkanes_1.alkanes.BlockResponse.deserializeBinary(new Uint8Array(Buffer.from((0, utils_1.stripHexPrefix)(hex), "hex"))).block).toString("hex"));
}
function decodeTraceBlockResponse(hex) {
    return alkanes_1.alkanes.TraceBlockResponse.deserializeBinary(new Uint8Array(Buffer.from((0, utils_1.stripHexPrefix)(hex), "hex"))).traces.map(({ outpoint, trace }) => {
        return {
            outpoint: {
...
function decodeTraceResponse(hex) {
    const resp = alkanes_1.alkanes.AlkanesTrace.deserializeBinary(new Uint8Array(Buffer.from((0, utils_1.stripHexPrefix)(hex), "hex")));
...
function decodeSimulateResponse(response) {
    const res = alkanes_1.alkanes.SimulateResponse.deserializeBinary(new Uint8Array(Buffer.from((0, utils_1.stripHexPrefix)(response), "hex")));
...
function decodeOutpointResponse(result) {
    return outpointResponseToObject(((protorune_1.protorune.OutpointResponse.deserializeBinary(new Uint8Array(Buffer.from(result.substr(2), "hex"))).toObject() || {}).balances || {}).entries || []);
}
...
function encodeAlkaneInventoryRequest(block, tx) {
    ...
    return ("0x" +
        Buffer.from(new alkanes_1.alkanes.AlkaneInventoryRequest(input).serializeBinary()).toString("hex"));
}
function decodeAlkaneInventoryResponse(hex) {
    const res = alkanes_1.alkanes.AlkaneInventoryResponse.deserializeBinary(new Uint8Array(Buffer.from((0, utils_1.stripHexPrefix)(hex), "hex")));
...
function encodeAlkaneStorageRequest({ id, path, }) {
    ...
    return ("0x" +
        Buffer.from(new alkanes_1.alkanes.AlkaneStorageRequest(input).serializeBinary()).toString("hex"));
}
function encodeAlkaneStorageRequestString({ id, path, }) {
...
function decodeAlkaneStorageResponse(hex) {
    const res = alkanes_1.alkanes.AlkaneStorageResponse.deserializeBinary(new Uint8Array(Buffer.from((0, utils_1.stripHexPrefix)(hex), "hex")));
