...
function parseTransaction(v) {
    const transaction = bitcoinjs_lib_1.Transaction.fromBuffer(Buffer.from((0, utils_1.stripHexPrefix)(v), "hex"), false);
    return transaction;
}
const padLeftEven = (n) => {
    if (n.length % 2 === 0)
...
function parseRunestoneFromTransaction(tx) {
    const script = bitcoinjs.script.decompile(tx.outs.find((v) => v.value === 0n).script);
